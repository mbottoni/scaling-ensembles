...
def load_pairwise(exp_dir: Path) -> tuple[list[tuple[float, float, int]], list[tuple[float, float, int]]]:
    """Returns (all_pairs, per_width_aggregates) for scatter+regression."""
    ...
    for width, pw in by_width.items():
        ...
        for r in pw:
            acc = (float(r["model_a_accuracy"]) + float(r["model_b_accuracy"])) / 2
            disagree = 1 - float(r["agreement"])
            all_pairs.append((acc, disagree, params))
        agg_acc = np.mean([(float(r["model_a_accuracy"]) + float(r["model_b_accuracy"])) / 2 for r in pw])
        agg_dis = 1 - np.mean([float(r["agreement"]) for r in pw])
        aggregates.append((agg_acc, agg_dis, params))
    return all_pairs, aggregates
...
def main() -> None:
    ...
    # all_pairs: for regression (individual comparisons where available)
    # agg_acc/agg_disagree: for plotting and annotation (per-width aggregates)
    all_pairs_acc, all_pairs_disagree = [], []
    agg_acc, agg_disagree = [], []

    # Plot pairwise data (CNN + ResNet on CIFAR-10)
    for exp_name, cfg in DATASET_CONFIG.items():
        pairs, aggregates = load_pairwise(OUTPUT_ROOT / exp_name)
        if not pairs:
            LOGGER.warning("No pairwise data for %s", exp_name)
            continue
        # Accumulate individual pairs for regression
        all_pairs_acc.extend([p[0] for p in pairs])
        all_pairs_disagree.extend([p[1] for p in pairs])
        # Plot per-width aggregates
        accs = [p[0] for p in aggregates]
        disagrees = [p[1] * 100 for p in aggregates]
        params = [p[2] for p in aggregates]
    ...
    # Plot summary data for other datasets (aggregate only)
    for label, (csv_path, color, marker, size) in SUMMARY_FILES.items():
        ...
        all_pairs_acc.extend(accs)
        all_pairs_disagree.extend([d / 100 for d in disagrees])
        agg_acc.extend(accs)
        agg_disagree.extend([d / 100 for d in disagrees])
    ...
    if all_pairs_acc:
        ...
        ax.plot([1 - x for x in x_line], p_poly(x_line),
                ...
                label=f"OLS fit (n={len(all_pairs_acc)} pairs): r = {r:.3f}")
